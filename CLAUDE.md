...
npm run cli -- report --narrate  # the same rebuild, plus one plain-language sentence per step written onto each bundle (no browser, no re-run)
npm run cli -- catalog cases.xlsx --run --report-lang th  # each case's own report page (linked from its name in the catalog index) and its narrative in Thai; en is the default, --no-case-narrative skips the one generator call per case
npm run cli -- catalog cases.xlsx --run --narrate  # narrate every step as the run goes (WOWLIDATOR_NARRATE=on is the same switch; off by default — one model call per case)
...
Each role is matched to a tier's strength, not to a favourite vendor: repair is small and latency-sensitive (Groq is fastest), generation sends the biggest prompt in the system (Gemini has the largest free context), navigation is one small structured decision per turn (Groq again — the loop, not the model, owns the reasoning; OpenRouter remains the natural re-point for a stronger agent model). Every role is re-pointable with two env vars — see `.env.example`. `PROVIDERS` in `src/config.ts` is the live list (fifteen as of 2026-09-07, including the `claude-cli`/`claude-tty`/`claude-cloud` family that shells out to Claude Code rather than calling an HTTP API); `LLM_ROLES` is the live role list.

**There were five roles; two were retired on 2026-09-11, both on the evidence of the usage ledger rather than on taste.**

`governor` advised the parallel queue and went with the whole queue-governor subsystem. Its own contract had promised it was *"an OPTIMISER, never a dependency: absent, disabled, erroring or out of budget, the deterministic scheduler runs exactly as it would alone"* — so removal could cost advice and never a verdict. Its default had already been a pure rules function since 2026-08-31, and the model half fired zero times in 18,704 recorded calls. See `src/orchestrator/CLAUDE.md`.

`data` was retired the same day. It reached a model only for a `fillRetry` step whose kind was `custom`, and the evidence said that never happened: across 1,429 authored flows not one `fillRetry` step was written at all, and every one of the 32 calls the usage ledger held under that role was `wowlidator doctor` proving the model id still resolved. `fillRetry` itself stays, with its kinds (`email`, `username`, `name`, `phone`, `text`) generating deterministically and costing nothing — see `src/data/mock-data.ts`. The lesson is the one worth keeping: a role that exists so escalation is *possible* still has to be reachable by something that writes flows, and this one was reachable only by hand.
...
- **An API provider's limits are obeyed before they refuse, and a refusal holds dispatch rather than sealing rows** (2026-09-11). A provider that states an in-flight ceiling (`PROVIDER_CONCURRENCY` in `src/config.ts`; emmiedev takes two calls per key) is entered through the same admission gate a local server uses, so lane three waits instead of being turned away. A refusal that arrives anyway is recorded against the provider (`src/providers/api-pressure.ts`) and `quotaHolding()` stops dispatch for as long as the provider asked — the API half of the claude-window hold, ORed into the same reading the lanes and the authoring pool already consult. Every API call lands on `.wowlidator/api-usage.jsonl` (`src/providers/api-usage-log.ts`); `claude-*` keeps its own ledger and never writes there. And a provider refusal is recorded as `providerRefused`, apart from `authoringRefused`, so it never counts toward the refusal cap: a busy minute must not cost a catalog a row that a `--resume` can no longer pick up.
...
- `src/data/CLAUDE.md` — Mock data and `fillRetry`; upload fixtures
...
- `src/generator/CLAUDE.md` — The case narrative; The authoring review; the suite's refusal memory; Authoring rails from a hand-authored comparison; Claim-fidelity rails; Negative testing; Boundary-value analysis; the tree's rendering, never the sheet's wording; code-grounded authoring; dead-end risk and error diagnosis; every step in plain language (`--narrate`); the Steps column is a script to perform (`scriptDemand`, `settleScriptDemand`); tokens are resolved, never typed (`value-rules.ts`); "only" means only (exclusivity); a persona hand-off is one `signIn`; the last word is a rewrite, not a refusal
...
- `src/monitor/` — the run-state projector both monitors read (`run-state.ts`);
  documented under `src/ui/CLAUDE.md`
- `src/orchestrator/CLAUDE.md` — What the agent claims is never the evidence; the flow-file script rung; form-filling actions; `dbCount`; progress and stall judges; the dependency wait the scheduler alone decides; why the queue governor was retired; the off-page allowance and re-activation rails; the agent runs as whoever is active
- `src/providers/CLAUDE.md` — Structured output on free tiers; Prompt discipline
- `src/repair/CLAUDE.md` — Runtime script evolution
- `src/reporter/CLAUDE.md` — Reading the report; Proof bundles and the report; the catalog report and its live rewrite; the target on every step; a rescued step shows once; a case gets its own page and the queries behind its DB evidence are on it
- `src/ui/CLAUDE.md` — The panel server; wowUI; the Ledger home page at `/`; the live monitor at `/monitor`; the usage cap; the Database card; run gates; re-authoring and the work queue; personas asked for from the panel (the account is remembered, the password never is); the console reads the output, it does not rewrite it
...
- `.claude/agents/healer-expert.md` — subagent for any change to the repair plane in `src/healer/` (the attempt loop, the heal prompt and selector-syntax rules, the echo check, the confidence gate, `#verify` and its per-action visibility bar, the AX capture and probe it prices, the healed-selector cache, the retrieved heal hints); it enforces "a cached repair is believed by every later run" and "the gate order is the design", and routes a repair the ladder should never have asked for back to the engine.
