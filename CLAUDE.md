...
**A context document can be cut to the part that bears on the case** — `src/catalog/retrieve.ts`, **off by default**, `--context-budget 24000` turns it on. The problem is real: `authorEachRow` authors each table row in its own call, so whole context documents are multiplied by the row count, and a twelve-row sheet with one 120,000-character spec spends ~360k input tokens on background, eleven-twelfths of it about rows that call is not writing. Selection is **BM25 over heading-aware chunks** — no model call, no dependency, no key, because the query is the row's own steps and the claims prompt already says to use the document's own words, so query and corpus share vocabulary by construction. Measured on a 60,000-character spec across four rows: 94% less background, each row quoting exactly its own section.

**It is opt-in because it decides what every authoring prompt sees**, and an inaccurate context poisons every test written from it — the same call `--probe`, `--agent-assist` and `--repair` make. The first cut shipped it on by default and that was wrong twice over: it was on, and it was broken. Five rails, four of which exist because a defect got past the first round of tests:
