...
| 11 | **Misattributed interference** | a cross-lane write makes lane A's failure look like an app defect | **Interference detector**: on any non-pass, check the ledger for another lane that wrote an intersecting section during the case's window; stamp `possible cross-case interference` on the bundle, feed it to the error-diagnosis as a signal, record the verdict as *provisional*, and re-run that one case alone once **after the rest of the plan** (one at a time, automatic; the replacement verdict overwrites the provisional one). The lane never waits in place: the first version drained the pool from inside the lane, and three lanes stamped together each waited for the other two — a hard freeze (2026-09-02, ec10NS) |
